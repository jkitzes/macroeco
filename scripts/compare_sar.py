...
The compare_sar analysis allows you to compare observed species-area
relationships (SAR) with any number of predicted SARs.  An SAR is a commonly
used macroecological metric which examines the number of species found in a
...
you would input the appropriate parameters and this analysis would divide the
plot into halves, fourths, and eighths and take the average number of species
across all of the smaller plots. Therefore, you can have a fractional average
number of species per areas. For additional information on SARs, please see the
provided references and the references therein.

OUTPUT

The compare_sar analysis outputs one folder per dataset, a logfile.txt, and, if
possible, a map of the location(s) of the dataset(s).  The folder has the name
sar_plots_compare_sar_* and contains a log(species) vs. log(area fraction) plot
as a .png file.  Area fraction means that the anchor area (largest area) for
the plot is 1 and all smaller subplots are fractions less than one. Each plot
will have the observed SAR and any SAR generated by a predicted SAR specified
in the predicted_SAR_curves parameter. In addition to this plot, this analysis
will output csv files with the same file name as the plot, but with the name of
the predicted SAR appended to the end.  These files contain the data for the
given SAR used to make the plot.  For example, if you choose two predicted SARs
and run the analysis, you will get three csv files: two with the predicted SAR
data and one with the observed data. With these files, you can re-plot the data
in anyway you chose.  Please note that the file names provide a detailed
...
if __name__ == '__main__':
    ...
    os.chdir(os.path.join(cwd,folder_name))
    fout = open('README_compare_sar', 'w')
    with fout:
